...
| Font Loading | `@nuxt/fonts` module | Manual `dns-prefetch`, `preconnect`, external stylesheet links |
...
- Material Symbols external link: NOT used (Phase 7 decision: Lucide-only)

**Conflict cleanup** (NEW v3): Grep for `fonts.googleapis.com` in `nuxt.config.ts` → must return ZERO
...
| Icons | `lucide-vue-next` | Component imports | ✅ Lucide-only (no Material Symbols, no inline SVG icons) |
