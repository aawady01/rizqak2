...
const variantClasses = computed(() => {
  const base = variantClassMap[props.variant] || "text-ds-body-r";
  const uppercase = props.variant === "h6" ? "uppercase" : "";
  const weightClass = props.weight ? `font-${props.weight}` : "";
  const colorClass = props.color || "";

  return [base, uppercase, weightClass, colorClass].filter(Boolean).join(" ");
});
</script>
...
<style scoped>
/* Mapping tokens to utility-like classes if not already handled by Tailwind 4 auto-detection */
/* Note: Tailwind 4 with @theme automatically creates utilities for --font-size-ds-* */
</style>
